...
func (d *Doc) mkList(name string, n int) error {
   ...
   if n >= len(d.Buf.Obj) {
      Goose.Fatalf(1,"Invalid object reference: %s",d.Buf.Obj)
   }

   ObjN := d.Buf.Obj[n]

   if ObjN == nil {
      Goose.Fatalf(1,"Null object, obj#:%d",n)
   }

   if ObjN.Header.Dic == nil {
      Goose.Fatalf(1,"No object header dic: %s",ObjN.Header)
   }

   if _, ok = ObjN.Header.Dic["Type"]; !ok {
      errors.New("Unknown type")
   }

   Goose.Logf(7, "Walking on object (%s) %s", typ, ObjN)

   switch ObjN.Header.Dic["Type"].(type) {
   case *NameT:
      typ = string(*(ObjN.Header.Dic["Type"].(*NameT)))
      Goose.Logf(7, "Walking on object of recognized type %s", typ)
   default:
      Goose.Logf(1, "Walking on object of type %s", ObjN.Header.Dic["Type"])
      return errors.New("Type info corrupted")
   }

   if typ == "Page" {
      Goose.Logf(4, "Found page %d", len(d.Index[name]))
      d.Index[name] = append(d.Index[name], ObjN)
      return nil
   }
   ...
   switch ObjN.Header.Dic["Kids"].(type) {
   case *ArrayT:
      kids = *(ObjN.Header.Dic["Kids"].(*ArrayT))
   default:
...
func SMWalk(buf *Buf, sz int64) {
   ...
   for (buf.StateLvl > 0) || (buf.State[buf.StateLvl] > 0) || (buf.Off < (sz - 6)) {
      if Hnd, ok = sm[buf.State[buf.StateLvl]][buf.Raw[buf.Off]]; ok {
         ...
         Goose.Logf(1, "Before undef hnd: %s", buf.Raw[buf.Off-100:buf.Off+100])
         Goose.Logf(1, "Before undef hnd hex: %2x", buf.Raw[buf.Off-100:buf.Off+100])
...
func New(Hnd io.Reader, sz int64) (*Doc, error) {
   ...
   //TODO: check size and panic if too big
   ...
   for err == nil {
      n, err = Hnd.Read(d.Buf.Raw[pos:])
      if err == nil {
         pos += n
      }
   }

   // Para fins de debug, vamos salvar uma copia do PDF
   // Em producao, devemos desabilitar o trecho abaixo

   err2 := ioutil.WriteFile("bkp.pdf", d.Buf.Raw, 0644)
   if err2 != nil {
      Goose.Logf(1,"PDF copy failed: %s",err)
   }

   // Fim do trecho que salva o PDF para debug
...
/*
   cat, err = d.Catalog()
   if (err == nil) && (cat != nil) {
      if entry, ok := cat.Dic["Pages"]; ok {
         switch entry.(type) {
         case *IndObjT:
            n = int(*(entry.(*IndObjT)))
            if (n < len(d.Buf.Obj)) && (d.Buf.Obj[n]!=nil) && (d.Buf.Obj[n].Header.Dic!=nil) {
