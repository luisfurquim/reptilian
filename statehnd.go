            ...
            if filter, ok := buf.DataStack[buf.StateLvl].(*ObjT).Header.Dic["Filter"]; ok {
               ...
               if err == nil {
                  ...
                  if err != nil {
                     ...
                     Goose.Logf(1, "Filter error (%s).1: error %s, %s", filter, err, buf.Raw[buf.Off-1:buf.Off+sz])
                     os.Exit(1)
                  }
               }
               ...
               if (err != nil) && (err != io.EOF) && (err != io.ErrUnexpectedEOF) {
                  ...
                  txt, err = ioutil.ReadAll(r)
                  if (err != nil) && (err != io.EOF) && (err != io.ErrUnexpectedEOF) {
                     ioutil.WriteFile("./comprimido", []byte(stream), 0777)
                     Goose.Logf(1, "Filter error (%s).2: error %s, %s", filter, err, buf.Raw[buf.Off-1:buf.Off+sz])
                     Goose.Logf(1, "Stream header data %s", buf.DataStack[buf.StateLvl].(*ObjT).Header)
                     os.Exit(1)
